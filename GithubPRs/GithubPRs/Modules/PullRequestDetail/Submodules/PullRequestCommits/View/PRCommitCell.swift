...
class PRCommitCell : UITableViewCell {
    ...
    var commit : PRCommitViewModel? {
        didSet {
            guard let commit = commit else { return }
            self.loginSpinner.startAnimating()
            photoImageView.loadImage(at: commit.avatarURL) { [weak self] in
                self?.loginSpinner.stopAnimating()
            }
            
            titleLabel.text = commit.message
            descriptionLabel.text = commit.description
            captionLabel.text = commit.caption
