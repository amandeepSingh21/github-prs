...
class PRCommentCell : UITableViewCell {
    //MARK: - Properties
    static let identifier = "PRCommentCell"
    
    ///Render cell with data
    var comment : PRCommentViewModel? {
        didSet {
            guard let comment = comment else { return }
            self.loginSpinner.startAnimating()
            photoImageView.loadImage(at: comment.user.avatarURL) { [weak self] in
