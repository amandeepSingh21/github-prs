//
//  MockPRCommentsInteractor.swift
//  GithubPRsTests
//
...
class MockPRCommentsInteractor: PRCommentsInteractorProtocol {
    ...
    private func proccess(_ request: Request, completion: @escaping PRCommentAPIResult) {
        switch self.viewStateToTest {
        case .noResultsFound:
            let mockData = MockCommentsData.getEmpty()
            completion(.success(mockData))
        case .loaded:
            let mockData = MockCommentsData.getNonEmpty()
            completion(.success(mockData))
        case .error:
